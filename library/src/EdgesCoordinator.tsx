...
const useOutflowSteps = (flow: Flow, network: Network) => React.useMemo(
    () => flow.outflow.map((outflow: any) => calcOutflowSteps(outflow, network.commoditiesMap)),
    [flow, network]
)

const useEdgesWithViewOpts = (flow: Flow, network: Network, strokeWidth: number, flowScale: number) => React.useMemo(
    () => {
        const grouped = groupBy(
            network.edgesMap,
            ({ from, to }) => JSON.stringify(from < to ? [from, to] : [to, from])
        )
        return flatten(
            map(grouped, group => {
...
export const EdgesCoordinator = (
    ...
    return <>
        {map(edgesWithViewOpts, ({ edge, translate, multiGroup }: { edge: Edge, translate: number, multiGroup: boolean }) => {
